...
Development sessions on this repo may themselves run through a redacting proxy. A secret-shaped literal (fake or real) in source/tests gets silently redacted from the AI's context and corrupts the file when it round-trips — this actually happened to `main_test.go`. Always build test tokens at runtime (see `key` in main_test.go). When editing near them, use scripted replacements, not copy-paste.
